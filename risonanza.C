...
//vector <double> errore_massimi;

//vector <double> errore_minimi;
...
/*            // Calcolo errore se possibile

double errore_min, errore_mas = 0.0;

        if (max_index > 0 && max_index + 1 < file.pendolo.size()) {

            double dx1 = abs(file.pendolo[max_index] - file.pendolo[max_index - 1]);

            double dx2 = abs(file.pendolo[max_index] - file.pendolo[max_index + 1]);

            errore_min = min(dx1, dx2); // oppure (dx1 + dx2) / 2

            errore_mas = max(dx1, dx2);

        }

        errore_massimi.push_back(abs(errore_mas));

        errore_minimi.push_back(abs(errore_min));
...
for(size_t j = 0; j < picchi.size()-1; j+=2) {

    periodi.push_back(picchi[j+1].first - picchi[j].first);
...
// dimezzo gli errori perchè stampo solo i massimo ogni 2

/*vector<double> err_mas, err_min;

for (int i = 0 ; i < errore_massimi.size(); i++ ){

if (i%2==0){

    err_mas.push_back(errore_massimi[i]);

}

}

for (int i = 0 ; i < errore_minimi.size(); i++ ){

if (i%2==0){

    err_min.push_back(errore_minimi[i]);

}

}

errore_massimi = err_mas;

errore_minimi = err_min;
...
fout << "La deviazione standard dei picchi minimi e' [rad]: " << stDevp(picchi_min)/sqrt(picchi_min.size()) << endl;
...
fout << "La STDev dei ampiezza pp delle oscillazioni " << stDev(semiamp_piccopicco)/sqrt(semiamp_piccopicco.size()) << endl;
...
fout << "Errore sul periodo: " << stDev(periodi)/sqrt(periodi.size()) << endl;

fout << "La stima di omega f e': " << 2*M_PI/media(periodi) << endl;

fout << "Errore di omega f e': " << 2M_PI(stDev(periodi)/periodi.size())/(media(periodi)*media(periodi)) << endl;
...
double errorewf = 2M_PI(stDev(periodi)/periodi.size())/(media(periodi)*media(periodi));
